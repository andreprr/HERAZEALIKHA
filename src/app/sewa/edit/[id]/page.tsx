...
export default function EditSewaPage() {
    ...
    const fetchSewaData = async () => {
      try {
        ...
        if (data) {
          ...
          setNoWa(data.no_wa);
          setJenisJaminan(data.jenis_jaminan);
          setNoJaminan(data.no_jaminan);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Update data utama (Pelanggan, Jadwal, Total Harga, Pembayaran)
      const { error: sewaError } = await supabase.from('sewa').update({
        nama_penyewa: namaPenyewa, no_wa: noWa, jenis_jaminan: jenisJaminan,
  ...
  return (
    <div className="flex flex-col gap-6 min-h-screen pb-24 pt-2 w-full max-w-5xl mx-auto overflow-x-hidden">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/sewa" className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white border border-pink-200 rounded-xl hover:bg-pink-50 shrink-0 text-sm shadow-sm transition-colors text-slate-600">
          <X size={16} /> <span className="hidden sm:inline">Batal</span>
        </Link>
      </div>

      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-pink-200 w-full">
        <form onSubmit={handleUpdate} className="space-y-8 w-full">
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-pink-600 border-b pb-2 border-pink-100">1. Data Penyewa</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={namaPenyewa} onChange={e => setNamaPenyewa(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={noWa} onChange={e => setNoWa(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-1">
                  ...
                  <select value={jenisJaminan} onChange={e => setJenisJaminan(e.target.value)} className="w-full px-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none text-slate-800 focus:border-pink-500 focus:ring-1 focus:ring-pink-500">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={noJaminan} onChange={e => setNoJaminan(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-pink-600 border-b pb-2 border-pink-100">2. Jadwal Sewa</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="date" value={tanggalBawa} onChange={e => setTanggalBawa(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5 text-slate-700">Status Transaksi</label>
                  <select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none font-medium text-slate-800 focus:border-pink-500 focus:ring-1 focus:ring-pink-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <Calendar className="absolute left-3 top-3 text-slate-400" size={16} />
                    <input type="date" value={tanggalKembali} onChange={e => setTanggalKembali(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-red-500 mb-1.5">Jam Kembali</label>
                  <div className="relative">
                    <Clock className="absolute left-3 top-3 text-slate-400" size={16} />
                    <input type="time" value={jamKembali} onChange={e => setJamKembali(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm font-semibold rounded-xl bg-pink-50 border border-transparent outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 text-slate-800" required />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-pink-100">
            <h3 className="text-sm font-bold text-pink-600 mb-4">3. Pesanan Barang</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4 items-end">
              <div className="flex-1 w-full">
                ...
                <div className="relative">
                  ...
                  <select value={selectedBarang} onChange={e => setSelectedBarang(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl bg-pink-50 border border-transparent outline-none truncate text-slate-800 focus:border-pink-500 focus:ring-1 focus:ring-pink-500">
              ...
              <div className="w-full sm:w-24">
                ...
                <input type="number" min="1" value={qtyPesan} onChange={e => setQtyPesan(Number(e.target.value))} className="w-full px-3 py-2.5 text-sm rounded-xl text-left sm:text-center bg-pink-50 border border-transparent outline-none text-slate-800 focus:border-pink-500 focus:ring-1 focus:ring-pink-500" />
              </div>
              <button type="button" onClick={tambahKeKeranjang} className="w-full sm:w-auto bg-pink-600 text-white font-semibold py-2.5 px-6 rounded-xl text-sm flex items-center justify-center gap-2 shrink-0 shadow-sm transition-transform hover:scale-[1.02] hover:bg-pink-700">
                <Plus size={16} /> Tambah
              </button>
            </div>

            <div className="rounded-xl border border-pink-100 overflow-hidden w-full">
                ...
                <table className="w-full min-w-[600px] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-pink-50 bg-white">
                    {cart.length === 0 ? (
                      ...
                      cart.map(item => (
                        <tr key={item.barang_id} className="hover:bg-pink-50/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right font-bold text-pink-600 whitespace-nowrap">Rp {item.subtotal.toLocaleString('id-ID')}</td>
          ...
          <div className="p-4 rounded-xl bg-pink-50/50 border border-pink-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
            {/* Total Harga */}
            <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={metodePembayaran} onChange={e => setMetodePembayaran(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm rounded-lg bg-white border border-pink-200 text-slate-800 outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" value={dp} onChange={e => setDp(Number(e.target.value))} placeholder="0" min="0" className="w-full pl-9 pr-3 py-2 text-sm rounded-lg bg-white border border-pink-200 text-slate-800 outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500" />
              </div>
            </div>

            <div className="flex flex-col items-start md:items-end md:border-l border-pink-200 md:pl-4 pt-2 md:pt-0 border-t md:border-t-0">
          ...
          <div className="pt-4 flex justify-end border-t border-pink-100">
            <button type="submit" disabled={isSubmitting || cart.length === 0} className="w-full sm:w-auto flex justify-center items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white font-bold py-3 px-8 rounded-xl text-sm disabled:opacity-50 transition-colors shadow-sm">
              <Save size={18} />
